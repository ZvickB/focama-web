...
- `POST /api/product/deep-dive`
  ...
  - when no lower exact offer is found, may show up to three deterministic "Similar options" from the already-fetched Google Shopping response. These use the normalized display name as the anchor, require a shared product-family signal plus a provable capacity, size, or generation difference, and link to Google Shopping (not an unverified merchant page).
