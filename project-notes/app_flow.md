...
- `POST /api/search/finalize`
  ...
  - removes non-new-condition listings before Prime filtering, Haiku selection, and ranking-preference composition unless the shopper explicitly requested that condition; `lowest_price` means the lowest-priced eligible new items by default
  ...
  - records body, cache, Haiku, token-scoped snapshot persistence, and true end-to-end server timing separately; snapshot persistence still completes before the response while production measurements determine whether moving it off-path would be worth the cross-instance/readiness tradeoff
