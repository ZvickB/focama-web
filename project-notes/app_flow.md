...
- Optional `SENSITIVE_IMAGE_SHADOW_ENABLED=true` analysis can submit the original URL of an image that deterministic moderation already chose to hide to Sightengine. It runs sequentially in the background (roughly one request per second), combines people counting with real/artificial-face detection, and logs only a proposed show/hide result. Missing credentials, provider errors, and uncertain results fail closed; TensorFlow.js is retained only for the local evaluator and is not a production fallback. The hook never restores the image or changes the API response and remains disabled by default while mannequin and partial-body accuracy are evaluated.
