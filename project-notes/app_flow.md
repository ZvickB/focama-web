...
- When the query clearly names a structured provider brand or an evidenced model, discovery enforces that identifier before preview or Haiku selection. Generic product terms and ambiguous short brand-like words remain broad searches. A high-confidence one-character brand typo plus matching product-title evidence is instead offered as an optional corrected search; it never filters, reranks, or replaces the original candidate pool. If fewer than four credible exact candidates remain, the existing first-finalize partial-shortlist recovery offers a refined search rather than padding with other brands or reference items.
