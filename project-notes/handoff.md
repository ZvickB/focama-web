...
- Run the Sightengine sensitive-image analyzer in controlled shadow mode against a much broader real Amazon set, especially headless mannequins, cropped/partial bodies, packaging photos, and small background people. Review every proposed `show`, keep dangerous false reveals at zero, and measure provider latency/operation use before deciding whether any reveal feature is viable. The old TensorFlow.js person/face/pose stack remains only as an offline comparison harness, not a Render fallback.
- Future TensorFlow.js evaluation option: if Render is upgraded to substantially more CPU/memory, it could run as a controlled background/batch worker, but it should not return to the live web request path without fresh resource measurements and explicit approval. A lower-cost alternative is an opt-in overnight evaluator run on the developer's personal computer. Previously sanitized cache entries cannot be used directly because hidden image URLs are intentionally cleared; either approach would need a separate access-controlled evaluation queue that temporarily retains original URLs, or a controlled provider re-fetch. Define retention, cleanup, retry, and concurrency limits before building this.
...
- Account-level ranking priority preference (balanced/price/brands/variety) is specced but not built: see `project-notes/account-priority-preference-plan.md`. Awaiting design review (Codex opinion) before implementation; depends on auth QA above.
