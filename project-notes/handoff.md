...
- Compare prices (formerly Deep Dive; 2026-07-08) is a feature-flagged, account-gated, user-triggered modal panel that does price comparison only — review synthesis/signals and the `gpt-5-mini` eligibility pass were removed; button visibility is now the deterministic prefilter alone. Stale Immersive cache refreshes before showing lower store offers and returns an honest `limited` state if the refresh fails. With `SIMILAR_OPTIONS_ENABLED=true`, a missing exact product match (or an exact match with no lower offer) may show clearly labelled broader options of the same product kind and their differences; these never imply an exact price match or saving, and accessory/part/book/condition-only listings are excluded. Before enabling broadly, create the Supabase tables, configure `DEEP_DIVE_ENABLED=true`, `SIMILAR_OPTIONS_ENABLED=true`, `SERPAPI_API_KEY`, and the `DEEP_DIVE_ALLOWED_DOMAINS_US/CA` allowlists, then run live review against at least 20 finalized products and tune the prefilter from logs.
