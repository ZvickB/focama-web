...
- Finish auth/history/preferences QA: verify sign up, sign in, forgot-password email and recovery-link password update, OAuth return if Google/Apple are enabled, session persistence, sign out, local-to-account history migration, remote save on finalize, reload persistence, delete, clear, second browser/device account history, and `user_preferences` RLS/read/write persistence. Keep the production Focamai origin allowed in Supabase Auth redirect URLs.
...
- Account-level ranking priority preference is implemented on experiment branch `account-ranking-preferences` for local review: signed-in account Preferences can save `balanced | price | lowest_price | brand | range`, finalize/enrichment receive the effective enum, non-balanced results show an indicator with `Show balanced picks`, and a one-time results hint opens Preferences/auth. `Prefer lower prices` keeps the strongest fit then favors lower-priced credible alternatives; `Lowest prices` sends only the compact basic-fit filter prompt, retains every matching candidate Haiku returns, then selects the six lowest-priced matches. Active-preference finalizes name the query and preference in the readable waiting state. The 2026-07-12 live 10-case evaluation is recorded in `plans/account-ranking-preference-live-evaluation-2026-07-12.md`; it found price directionally useful but inconsistent, while brand/range often return the balanced set unchanged. Before merging broadly, repeat evaluation with both new price modes, complete live auth/RLS QA, and apply `20260713150100_add_lowest_price_ranking_preference.sql` to production after the original table migration.
