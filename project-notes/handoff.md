...
- Account-level ranking priority preference is implemented on experiment branch `account-ranking-preferences` for local review: signed-in account Preferences can save `balanced | price | brand | range`, finalize/enrichment receive the effective enum, non-balanced results show an indicator with `redo balanced`, and a one-time results hint opens Preferences/auth. Before merging broadly, create the `user_preferences` table, test the UI feel across mobile/desktop, run live auth/RLS QA, and decide whether the results hint/indicator copy feels too visible.
