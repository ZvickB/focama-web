...
- The homepage now preconnects Google Fonts in `index.html`, preconnects the configured backend origin, gives the hero wordmark higher fetch priority, and prefetches the results plus modal chunks immediately after `HomeExperience` mounts.
...
- Frontend auth and account-backed history are implemented: `AuthProvider`, lazy Supabase browser client, sign-in/create-account modal, Google sign-in button, header sign-in/sign-out UI, signed-out local history, signed-in Supabase `saved_searches`, and local-to-account migration. Search is still ungated.
...
- Finalized result rows, selected-product panels, grid/card view, and modal headings now accept a validated async `displayTitle` from mini enrichment so Amazon keyword stuffing does not dominate the UI. The provider title remains preserved as `sourceTitle` and appears behind a quiet full-title disclosure when it differs; unsafe normalization falls back to that source title.
...
- Production frontend calls use the configured Render origin by default. Browser-level network failures retry once through explicit same-origin Vercel rewrites for search, diagnostics, health, analytics, feedback, and ping routes. Successful proxy use is persisted across visits; each homepage load also sends a direct Render health ping and clears the preference when direct access works again. HTTP errors and aborts do not fall back.
- Render CORS explicitly accepts the current `focamai.com` and `www.focamai.com` frontend origins and still tolerates the older `focama.vercel.app` origin for direct development/debug requests.
...
- Oxylabs is retired from the active stack. Keep any historical Oxylabs notes archived only.
...
- `POST /api/search/finalize`, `GET /api/search/enrichment-stream`, `GET /api/search/enrichment`, `GET /api/search/query-quality`, `GET /api/search/product-details`, `POST /api/search/retry-advice`, and disabled-by-default `POST /api/product/price-check` are wired in the Render app.
...
- Skipped-refinement preview modals now hydrate product detail bullets/description one product at a time from that cache or the Rainforest-first detail provider when opened, without showing AI analysis loading dots because finalize/enrichment has not run.
...
- Fresh product-detail hydration uses Rainforest for shortlisted ASINs and skipped-refinement preview modals.
...
- Mini enrichment writes `fit_reason`, `caveat`, `sourceTitle`, validated `displayTitle`, and a structured `matchIdentifier` back into the token-scoped session snapshot for the exact active `discoveryToken`, then the frontend hydrates results via SSE first and polling fallback second. Provider identity fields outrank deterministic extraction; AI may normalize descriptive fields but cannot supply authoritative product codes.
- The unused manual CA/CAD `POST /api/product/price-check` path remains isolated behind `PRICE_COMPARISON_ENABLED=false`. Automatic price intelligence is now a hero-only hybrid behind `HYBRID_PRICE_INTEL_MODE=off`: one cached Serper request discovers promising savings, then at most one SerpApi Shopping plus one Immersive Product request verifies a unique product group, selected variant, current store price, and direct retailer URL. CA/CAD and US/USD are supported only with explicit per-market domain allowlists. Exact source-visible variants, non-marketplace/new condition, currency/savings bounds, retailer-domain agreement, public DNS, and a bounded same-domain redirect probe are mandatory; Haiku cannot override missing deterministic proof. Shadow mode never emits, surface mode also requires percentage rollout, SerpApi defaults to 32 calls/day, identity judgments cache for 7 days, and price/link data cache for 30 minutes. The modal-only panel, fail-silent behavior, Amazon CTA hierarchy, and no-card-badge decision remain unchanged.
- Post-implementation Canadian shadow replay on 2026-06-22 verified 0/8 comparisons: 7 completed quietly, one hit the 15-second pipeline timeout, and Shopping ambiguity or one-store Immersive responses prevented direct candidate mapping. This is safe but has no useful coverage yet, so production mode and rollout percentage must remain off.
- The Phase 3 schema is applied in the active Supabase project: `product_details_cache.provider_identity` and server-only `price_comparison_cache`; local JSON fallback remains development resilience. The canonical schema is documented in `project-notes/db-needs.md`.
