...
- Search now has centralized deterministic content moderation for the current frum-default policy. Clearly explicit queries stop before discovery/refine provider work; blocked adult/explicit products never enter preview or finalize; sensitive-but-legitimate listings stay available as text with `Image hidden for sensitive content` instead of a product image. Cache reads are re-moderated, Rainforest brand/category fields are preserved when present, and no paid moderation service or image-reveal setting is part of v1.
