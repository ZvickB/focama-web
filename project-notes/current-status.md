...
- Price-comparison Phase 3 backend infrastructure now exists behind `PRICE_COMPARISON_ENABLED=false`: the CA/CAD-only `POST /api/product/price-check` route validates the finalized candidate from server-owned token state, uses optional Supabase bearer auth for server-side offer redaction, applies endpoint-specific shared rate limits, and reuses 7-day match plus 30-minute price cache entries before provider calls. Separate Serper price intelligence is implemented behind `SERPER_PRICE_INTEL_ENABLED=false`: Serper Shopping, isolated Haiku same-product judgment, post-enrichment orchestration, 30-minute result and 7-day judgment caches, per-search/global budget controls, stale-selection protection, measurement logs, and SSE/poll delivery. A quiet modal-only `Better price found` panel now renders a valid result for the open product below product notes and above the unchanged Amazon CTA; invalid/missing results stay invisible with no loading state, and cards have no badge. The feature remains off by default while real-search accuracy, nudge rate, savings, and cost are measured.
