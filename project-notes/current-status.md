...
- Search failure diagnostics now exist outside Sentry: each guided search reuses the frontend search ID as a support code, failed searches show a support-code panel with safe copyable debug info and an optional filter/VPN question, and frontend/backend lifecycle events write best-effort rows to Supabase `search_attempts` and `search_events` when configured.
