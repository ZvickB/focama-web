...
- Active Amazon marketplaces are US and Canada (Associates-tagged), plus the untagged major stores UK, Germany, France, Italy, Spain, Australia, Japan, India, Mexico, and Brazil. Untagged stores search and click out on the shopper's selected local Amazon site but earn no commission. Do not add OneLink yet or let it override an explicit store selection. When a store-specific Associates tag exists, add it to `DOMAIN_TO_AFFILIATE_TAG` and remove that domain from `ACTIVE_UNTAGGED_AMAZON_DOMAINS`.
