...
- Price-comparison Phases 1-3 remain behind `PRICE_COMPARISON_ENABLED=false`: the unused manual CA/CAD endpoint keeps its isolated SerpApi/BlueCart path and cache. The automatic experiment is now a hero-only Serper → SerpApi Shopping → Immersive Product hybrid behind `HYBRID_PRICE_INTEL_MODE=off`. Serper supplies cheap candidate signals; SerpApi is called only for a promising survivor to prove the exact product/variant and obtain a direct approved-retailer URL. Deterministic identity, currency, condition, savings, marketplace, public-DNS, retailer-domain, and redirect-probe requirements fail closed; Haiku may reject semantic mismatches but cannot replace missing proof. `shadow` measures without emitting, `surface` also requires a nonzero rollout percentage, and SerpApi is capped at 32 calls/day by default. The quiet modal-only panel and Amazon CTA hierarchy are unchanged, and no card badge exists.
- Live QA on 2026-06-22 found that Serper is useful as a low-cost Shopping discovery layer but its returned offer links resolved to Google Shopping rather than dependable direct merchant pages, and four comparisons lacked sufficient variant proof. Detailed inputs and outcomes are preserved in `temp-data/price-intel-reviews/serper-live-review-2026-06-22.md` and `.json`.
- A hybrid shadow replay of those inputs after implementation completed 7/8 cases, timed out safely on one, and verified zero offers: Shopping ambiguity and one-store Immersive responses prevented Serper-to-store mapping. No uncertain result passed. Keep the feature off; the local report is `temp-data/price-intel-reviews/hybrid-live-review-latest.json`.
- The hybrid direction above is implemented but not enabled for production. The next gate is shadow/live review, including US cases and strict direct-link probes. Google Custom Search is not part of this path.
