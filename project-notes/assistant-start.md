...
## Test strategy checkpoint
- The normal `npm test` baseline is 243 enabled behavior-focused tests across 47 passing files. Deployment/live backend smoke (5 scenarios), cache-latency UI smoke (2 scenarios), and paid recommendation-quality evaluations are deliberately separate; do not restore exact prompt wording, trivial helper, or duplicated route-shell coverage just to increase the count.
- The paid read-only selector benchmark is `npm run test:benchmark:selector-models`. Its two-run 2026-09-10 comparison favored `gpt-5.6-terra` low reasoning over Haiku and Claude Sonnet 5 for quality with close p95 latency. Guided finalize now defaults to Terra through `OPENAI_SELECTOR_MODEL`, with Haiku as the automatic provider fallback.
...
- Product shortlists normally target 6 items. During the current tester rollout, a first finalize with fewer than 4 AI-identified strong fits returns only those credible picks and offers a more specific AI-proposed search instead of padding to six. On web, accepting that search preserves the existing answer/notes and automatically discovers and finalizes replacement picks without another refinement question.
...
- A 2026-09-10 read-only Supabase-backed ten-pool evaluation found that partial-shortlist recovery is not yet a reliable quality signal: it appeared for only 1/5 judge-identified weak sets. Its one Moto refined query added `unlocked` and disabled the narrow trailing-year/model guard by placing words after `2024`, worsening the same-pool proxy. Preserve active deterministic constraints across any refined search before relying on automatic recovery; do not infer that a missing recovery card proves four suitable products exist. See `project-notes/plans/haiku-recovery-quality-smoke-2026-09-10.md`.
- Discovery uses Rainforest API for active Amazon discovery. Oxylabs has been archived and is not an active fallback.
- Finalize rebuilds the candidate pool server-side from guided cache. Guided discovery and finalize cap that pool at 30 candidates, deliberately raised from 20 on 2026-05-01 so the AI selector can consider more of Rainforest's typical result set without a meaningful latency increase.
- Before AI selection, a conservative server-owned guard filters the whole condition-eligible pool for explicit currency price ceilings, a narrow allowlist of explicitly negated product/material terms, and trailing four-digit year/model signatures; missing price/model proof fails closed, and every rejection is recorded. `gpt-5.6-terra` then defines the AI-ranked shortlist frontier through short candidate indices and strict Responses API structured output; Haiku uses the equivalent tool contract on fallback: up to six high-confidence primary picks followed by ranked high-confidence reserves that the prompt instructs to satisfy the same explicit shopper requirements. The backend maps indices to server-owned candidate IDs and may promote only returned reserves; it returns fewer picks when the frontier runs out instead of padding from raw Rainforest order. The guard prevents the demonstrated Doona, over-budget briefcase, case/accessory, and wrong Moto-year/model failures, but it deliberately does not parse arbitrary semantic requirements; a structured semantic proof layer was considered and explicitly deferred by the user. During the current tester rollout, if the first finalize returns fewer than 4 strong AI fits plus a valid distinct suggested query, the backend returns only those picks and offers a self-contained, at-most-80-character refined search. The web recovery card shows preserved details and accepting it automatically discovers and finalizes replacement picks; shown, accepted, and kept-partial-picks outcomes are stored as search analytics events. Balanced ranking keeps the original order: inferred product fit, quality confidence (rating, review count, trustScore, and recognized category brand), price/value, shortlist variety, then raw Amazon position. The account-ranking experiment can send `rankingPreference: balanced | price | lowest_price | brand | range`; backend enum validation defaults unknown/missing values to balanced. Preferences shape which frontier picks are selected: `price` keeps the strongest contextual fit then favors lower-priced credible alternatives; `lowest_price` sends a dedicated fit-only filter prompt, retains every match it returns, then deterministically selects the six lowest-priced matches; brand fills credible known brands before fitting non-brand alternatives; and range seeks both price and feature/style differences. Luna enrichment receives the effective preference.
- Cosmetic variants of the same identified product model are removed before AI selection and after its ordered shortlist; unique selector picks retain their order and deterministic replacements append at the end. Standalone numbers are not treated as model-identity evidence, so shared measurements or quantities do not collapse distinct products. The selector returns a strict binary specific-brand decision and a per-pick brand label. When false, final composition deterministically favors no more than two distinct models per resolved brand where credible alternatives exist, then relaxes that preference rather than leave slots empty. Provider `brandName` wins; the selector's label covers selected products when Rainforest leaves it blank. A clear named-brand query overrides an incorrect false decision; different models, generations, capacities, widths, and major feature tiers stay distinct.
- Structured provider brands and compact alphanumeric model identifiers are hard constraints. Plain-language model names and product descriptors stay in the matching-brand candidate pool for the AI selector to rank because marketplace titles often use synonyms or singular/plural variants. A one-character brand spelling near-match paired with matching product-title evidence instead shows an optional corrected search and never alters the original candidate pool, preview, or shortlist; this avoids silently replacing a potentially correct uncommon term.
...
- Refine uses `gpt-5.6-luna` with low reasoning to generate a primary and distinct alternate question together, each with four matching direct answers including a neutral choice. `OPENAI_REFINEMENT_MODEL` is the explicit override and Haiku remains the failure fallback. The UI shows one pair at a time; `Get a different question` swaps both after a 900 ms breathing-dot transition without clearing the separate optional notes box. The selected answer and notes are combined only when finalizing.
