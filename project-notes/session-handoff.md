...
- Exact structured provider brands and compact alphanumeric model identifiers hard-filter discovery. Plain-language model names and product descriptors remain in the matching-brand pool for Haiku to rank because marketplace titles often use synonyms or singular/plural variants; this specifically prevents `undershirt` from excluding an exact Hanes listing titled `T-Shirt`. A one-character brand spelling near-match with matching product-title evidence only produces an optional corrected-search card; it never alters the original candidate pool or selection. This protects correct but uncommon user terms and rejects false matches such as `apple sauce` → Apple electronics.
- `GET /api/search/refine` defaults to `gpt-5.6-luna` with low reasoning and returns a primary short follow-up question and one distinct alternate, each with four matching direct answers in the same response; Haiku remains the failure fallback.
...
- OpenAI refinement, query-quality review, and retry advice now default to `gpt-5.6-luna` with low reasoning; `OPENAI_REFINEMENT_MODEL` is the explicit override. A paired three-query paid smoke measured a 3.86s Luna mean versus a 9.96s Mini mean with coherent structured output from both. Async enrichment also defaults to Luna for product explanations and Improve Picks suggestions, with `OPENAI_FINALIZE_MODEL` as its explicit override. Haiku shortlist locking is unchanged.
- The Anthropic TypeScript SDK is now `0.122.0` (from `0.91.1`). The active Haiku model and shortlist request contract are unchanged; a paid three-pool Haiku/Luna smoke passed after the upgrade.
- A counterbalanced paid payload benchmark rejected two latency optimizations for now: compact Haiku input reduced tokens by 35.7% but was not faster when call order reversed and materially changed some picks; single-question Luna refinement reduced tokens by 65.3% but saved only about 0.25s across both orders while removing the ready alternate. Keep both production contracts unchanged. See `project-notes/plans/ai-payload-benchmark-2026-08-31.md`.
