...
- Priority 7 is now implemented in the first pass: retry is one expandable `Improve these picks` card directly after the shortlist, with one freeform correction field and `Update my picks`. Submission locks the editor, and the improved query is shown read-only in the results viewport while the safe refreshed search runs; there is no editable `Next search` confirmation step, quick-correction chips, separate retry panel, or floating retry prompt.
...
- Frontend auth shell is implemented: `AuthProvider`, `useAuth`, lazy Supabase browser client, `AuthModal`, header sign-in/sign-out UI, Google OAuth, account Preferences UI for shortlist ranking priority, and email/password recovery through Supabase forgot-password email plus a `PASSWORD_RECOVERY` new-password state. Apple sign-in remains intentionally hidden until its Developer configuration is ready. Search remains ungated. Signed-out history uses localStorage; signed-in history uses Supabase `saved_searches`; local entries migrate into the account on login. Live QA of Google OAuth return behavior, recovery email/link behavior, `user_preferences` RLS, and auth/RLS/history persistence is still pending.
