...
- Improve Picks retry parity: web now follows mobile by skipping the normal new-search refinement question and auto-finalizing as soon as refreshed discovery is ready. The web-only rollback switch is `VITE_AUTO_FINALIZE_RETRY_SEARCH=false` (default is enabled); this switch affects only Improve Picks retry advice, not ordinary searches, failed-search recovery, or partial-shortlist recovery.
